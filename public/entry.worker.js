...
(() => {
  ...
  // app/entry.worker.ts
  var NetworkFirstLoader = class extends Strategy {
    _handle(request, handler) {
      const networkRequest = handler.fetchAndCachePut(request);
      return new Promise((resolve) => {
        networkRequest.then((networkResponse) => resolve(networkResponse)).catch((_err) => {
  ...
  function debug(...messages3) {
    if (true) {
      console.debug(...messages3);
    }
  }
  ...
  function matchAssetRequest({ request }) {
    return isMethod(request, ["get"]) && ["/build/", "/icons"].some(
      (publicPath) => request.url.includes(publicPath)
    );
  }
